...
// parse → payloads → quote → prices/decimals → bid decision → pyth fee.
// Lifecycle concerns (exposure holds, target-block timing, sending, receipts) stay in the loops.
...
import { decideBid, type BidContext, type BidDecision } from "./strategy/index.js";
...
/**
 * Evaluate one open order end-to-end. Returns null when the order is skipped for a *policy* reason (no path,
 * no spread, caps, …) — the skip is logged and counted here. Unexpected failures (RPC, payload feeds) throw;
 * the calling loop owns error accounting.
 */
export async function prepareFill(deps: PrepareDeps, order: OpenOrder, baseFeeWei: bigint): Promise<PreparedFill | null> {
  ...
  return { parsed, payloads, quote, ctx, decision: result.decision, pythFeeWei: pythFeeFor(config, payloads) };
}
